...
public class J05056_Xep_hang_van_dong_vien_2 {
    public static void main(String[] args) {
        ...
        list.get(0).rank = 1;
        for (int i = 1; i < n; i++) {
            list.get(i).rank = list.get(i).getFinalAchievement() == list.get(i - 1).getFinalAchievement()
                    ? list.get(i - 1).rank
                    : i + 1;
        }
...
class Athlete {
    ...
    public String toString() {
        return id + " " + name + " " + secondsToString(getAchievement()) + " " + secondsToString(getPriority()) + " "
                + secondsToString(getFinalAchievement()) + " " + rank;
    }
